...
**생산계획(Production Planning) 구조**는 기업의 비즈니스 목표와 고객 수요를 바탕으로 <span class='hl'>"무엇을, 얼마나, 언제, 어떻게 생산할 것인가"를 결정하는 단계별 수립 체계</span>이다.
...
1. **총괄생산계획(S&OP / APP)**
    ...
    * **주요 내용**
    ...
    !!! note "APP 핵심 전략"
        ...
        추종전략과 평준화전략은 중기(6개월~18개월) 관점에서 변동하는 수요에 **인력과 재고를 어떻게 조절하여 대응할 것인가**를 다루는 생산관리 전략이다.
        
        **추종전략(Chase Strategy)**
        
        수요의 변동 추이에 따라 생산율이나 인력 수준을 직접 맞추어 나가는 전략이다.
        
        * **운영 방식**: 수요가 많을 때는 고용 및 잔업을 늘리고, 수요가 줄어들면 해고나 단축근무를 실시함
        * **장점**: 재고를 거의 보유하지 않아 재고 유지 비용이 최소화됨
        * **단점**: 잦은 고용 및 해고로 인한 채용·해고 비용 발생, 작업자 사기 저하, 품질 불안정이 나타남
        
        **평준화전략(Level Strategy)**
        
        수요 변동과 관계없이 일정 기간 동안 일정 수준의 생산율과 인력을 균일하게 유지하는 전략이다.
        
        * **운영 방식**: 수요가 적을 때는 남는 생산량으로 재고를 쌓고, 수요가 많을 때는 미리 쌓아둔 재고를 차감하여 대응함
        * **장점**: 고용 안정성이 확보되고 생산 프로세스가 안정되어 품질 및 생산성이 높음
        * **단점**: 수요가 적은 기간 동안 재고를 보관해야 하므로 재고 유지 비용이 커짐
...
3. **자재소요계획(MRP, Material Requirements Planning)**
    ...
    * **계획 단위**: 구성 부품, 반제품, 원자재
    * **주요 내용**
        * MPS에서 확정된 완제품 생산 계획과 **자재명세서(BOM, Bill of Materials)**, 현재 재고 데이터를 바탕으로 필요한 부품의 종류, 수량, 발주 시점을 산출한다. (입력: MPS, BOM, IRF)
        * 조달 리드타임을 고려하여 내작 <span class='hl'>생산 지시서 및 외주 발주서를 작성</span>한다.

    !!! note "MRP 핵심 3대 입력 자료"  
...
!!! note "생산계획과 일정관리"  
    ...
    APP/MPS/MRP/PAC는 생산계획 및 통제 기준 분류이고 대일정/중일정/소일정은 일정 관리 기준 분류이다. 각 구분은 유사성이 있으나 1:1로 대응되지는 않는다.
    
    | 구분     | APP/MPS/MRP/PAC          | 대일정/중일정/소일정         |
    | ------ | ------------------------ | ------------------- |
    | 분류 기준  | 계획 기능(Planning Function) | 계획 기간(Time Horizon) |
    | 관점     | 무엇을 결정하는가                | 언제까지를 계획하는가         |
    | 초점     | 의사결정 단계                  | 시간적 범위              |
    | ERP 기준 | 생산계획 모듈                  | 일정계획 수준             |
    
    공정관리 일정계획에서 대일정/중일정/소일정 구분은 다음과 같다.  
    
    | 구분                           | 일반적인 설명                      |
    | ---------------------------- | ---------------------------- |
    | 대일정(Master Schedule)         | 제품 또는 주문 전체를 대상으로 한 일정       |
    | 중일정(Operation/Shop Schedule) | 공정 또는 작업장 단위의 일정             |
    | 소일정(Detailed Schedule)       | 개별 작업(Job, Operation) 단위의 일정 |
...
**생산통제(Production Control)**는 확정된 생산계획(MPS, MRP, 상세 일정계획 등)을 바탕으로 현장에서 물리적인 생산 작업이 시작된 이후, <span class='hl'>계획대로 차질 없이 진행되고 있는지 모니터링하고 일탈 발생 시 즉각 보정하는 공정 관리 활동</span>이다.
...
```mermaid
flowchart LR
    A["1.라우팅<br>(Routing)"] --> B["2.일정계획<br>(Scheduling)"] --> C["3.작업지시<br>(Dispatching)"] --> D["4.진도관리<br>(Follow-up)"]
```
...
**MES(Manufacturing Execution System, 제조실행시스템)**는 <span class='hl'>공장 현장의 생산 활동을 실시간으로 감시하고 제어하여 생산 효율을 최적화하는 시스템</span>이다. 전통적인 생산 현장의 수기 관리 방식에서 벗어나 자재 투입부터 최종 제품 생산까지의 전 과정을 디지털화하여 관리한다.
...
flowchart TD
    ...
    A --> B["계획 기능<br/>(Planning)<br/>생산목표 달성을 위한 사전 계획"]
