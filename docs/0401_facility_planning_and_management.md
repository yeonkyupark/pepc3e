...
2. **실효능력(Effective Capacity)**

    현실적인 운영 조건과 제약 요인을 반영하여 실제로 기대할 수 있는 최대 생산 능력을 뜻한다.
    
    * **특징**: 제품 구성, 예방 보수, 작업자 휴식 시간, 품질 검사 시간 등이 차감된 기준임

3. **실제산출량(Actual Output)**

    특정 기간 동안 설비를 실제로 가동하여 달성한 실제 생산량을 뜻한다.
    
    * **특징**: 예상치 못한 설비 고장, 원자재 수급 차질, 작업자 불참 등으로 인해 실효능력보다 적게 나타나는 경우가 많음
...
1. **이용률(Utilization Rate)**
    ...
    $$\text{이용률} = \frac{\text{실제출력}}{\text{설계능력}} \times 100(\%)$$
...
1. **효율성(Efficiency Rate)**
    ...
    $$\text{효율성} = \frac{\text{실제출력}}{\text{실효능력}} \times 100(\%)$$
...
기업의 산업 특성과 경영 전략에 따라 완충용량의 크기를 다르게 설정한다.

1. **양의 완충용량(Positive Cushion)**
