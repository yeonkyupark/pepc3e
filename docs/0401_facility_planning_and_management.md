...
3. **트래킹 전략(Tracking Strategy, 추종전략)**
...
완충용량(또는 용량 쿠션, Capacity Cushion)은 예상되는 평균 수요를 초과하여 확보해 두는 설비용량의 여유분을 의미한다.

$$\text{완충용량(\%)} = 100(%) - \text{설비 이용률(\%)}$$
...
!!! note "완충용량과 규모의 경제 사이의 관계"

    최적 설비용량을 결정할 때는 규모의 경제와 완충용량의 균형이 중요하다.
    
    * **최적 운영 규모(Best Operating Level)**: 단위당 평균 생산 비용이 최소화되는 지점임
    * **전략적 균형**: 완충용량을 너무 크게 두면 규모의 경제 효과가 낮아지고, 완충용량을 완전히 없애면 규모의 불경제 영역(과부하)으로 진입할 위험이 높아짐
