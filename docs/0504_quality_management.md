...
품질관리 목적은 단순히 불량을 줄이는 것이 아니라 고객 만족과 기업 경쟁력을 향상시키는 것이다. 품질은 QCD(Quality, Cost, Delivery) 중 가장 우선되는 요소로 인식된다.
...
!!! note "Juran Trilogy"  
    ...
    ![](https://www.juran.com/wp-content/uploads/2019/10/juran_diagrams_the-juran-trilogy-sm.png)
...
Juran과 Feigenbaum은 품질비용(Cost of Quality, COQ)를 다음과 같이 구분한다. 공장관리에서는 예방비용을 증가시켜 실패비용을 감소시키는 것이 경제적이다.
...
품질비용(COQ, Cost of Quality)은 제품이나 서비스의 완벽한 품질을 달성하기 위해 지출되는 비용과 불량으로 인해 발생하는 모든 손실 비용을 포함하는 품질 관리 개념이다.

### 분류 체계(PAF 모델)

품질비용은 전통적으로 예방(Prevention), 평가(Appraisal), 실패(Failure)의 PAF 모델로 분류한다.

![](https://github.com/user-attachments/assets/b229b68d-9a71-4398-8c37-ba139a36c23a)

1. **통제 비용(Cost of Control)**

    좋은 품질을 확보하고 불량을 미리 방지하기 위해 정당하게 투입되는 적합 비용이다.
    
    * **예방비용(P-Cost, Prevention Cost)**: 불량 발생 자체를 사전에 막기 위해 지출하는 비용이다.  
        * **주요 항목**: 품질 계획 수립, 작업자 품질 교육, 설비 예방 보전, 공정 개선 활동    
    
    * **평가비용(A-Cost, Appraisal Cost)**: 제품이나 서비스가 요구 품질 기준에 적합한지 검사하고 측정하는 비용이다.  
        * **주요 항목**: 수입 검사, 공정 및 완제품 검사, 검사 장비 교정 및 유지비

2. **실패 비용(Cost of Failure)**

    품질 기준을 충족하지 못해 발생하는 비적합 비용으로, 불량이 발생했기 때문에 납부해야 하는 손실 비용이다.
    
    * **내부실패비용(IF-Cost, Internal Failure Cost)**: 제품이 고객에게 출하되기 전 공정 내부에서 발견된 불량으로 인한 비용이다.  
        * **주요 항목**: 폐기(Scrap), 재작업(Rework), 재검사 비용, 공정 대기 손실 
    
    * **외부실패비용(EF-Cost, External Failure Cost)**: 제품이 고객에게 전달된 후 현장에서 발견되어 발생하는 손실 비용이다.  
        * **주요 항목**: 무상 A/S 및 보증 수리, 리콜(Recall) 비용, 반품 처리, 기업 이미지 실추 손실

예방 및 평가에 투자하는 비용과 실패 비용 사이에는 상반된 교환(Trade-off) 관계가 성립한다.

* **품질 수준 향상 시**  
    예방비용(P-Cost)과 평가비용(A-Cost)을 늘리면 초기 투입 비용은 증가함
* **실패 비용 감소**  
    불량률이 줄어들면서 내부실패비용(IF-Cost)과 외부실패비용(EF-Cost)이 급격히 감소함
* **최적 품질비용점**  
    총 품질비용(Total COQ)이 최소가 되는 지점에서 경제적으로 최적의 품질 수준이 결정됨

!!! note "PAF 비용 요소 비교"

    품질 관리 단계에 따라 투자되는 비용 종류와 발생 시점이 달라진다.
    
    | 구분 | 예방비용(P-Cost) | 평가비용(A-Cost) | 내부실패비용(IF-Cost) | 외부실패비용(EF-Cost) |
    | --- | --- | --- | --- | --- |
    | **발생 시점** | 공정 설계 및 시작 전 | 공정 진행 및 검사 시 | 출하 전(공정 내) | 출하 후(고객 전달 후) |
    | **성격** | 불량 방지 목적의 투자 | 품질 수준 측정 목적 | 공정 내부 손실 | 최악의 손실 및 신뢰도 하락 |
    | **대표 예시** | 품질 교육, 설비 점검 | 제품 검사, 교정 측정 | 폐기, 재작업 | 리콜, 무상 수리, 반품 |
...
flowchart TD
    ...
    B["통계적 공정관리<br/>(SPC : Statistical Process Control)"]:::category
    C["샘플링 검사<br/>(Sampling Inspection)"]:::category
    D["실험계획법<br/>(DOE : Design of Experiments)"]:::category
    ...
    B1["관리도<br/>(Control Chart)"]:::detail
    B2["공정능력 분석<br/>(Cp, Cpk)"]:::detail
...
```mermaid
flowchart LR
    classDef start fill:#FFEBEE,stroke:#D32F2F,stroke-width:2px,color:#000,font-weight:bold;
    ...
    A["불량 증가 발생<br/>(Defect Increase)"]:::start

    B["① 파레토 분석<br/>(Pareto Analysis)<br/><br/>어떤 불량인가?"]:::process

    C["② 특성요인도<br/>(Fishbone Diagram)<br/><br/>원인은 무엇인가?"]:::process

    D["③ 관리도<br/>(Control Chart)<br/><br/>공정이 안정적인가?"]:::process

    E["④ 공정능력 분석<br/>(Cp / Cpk)<br/><br/>규격 만족 가능한가?"]:::process

    F["⑤ 실험계획법<br/>(DOE)<br/><br/>최적 조건은 무엇인가?"]:::process

    G["⑥ 표준화 및 관리<br/>(Standardization & Control)"]:::final
...
xychart-beta
    ...
    x-axis "시간" [1,2,3,4,5,6]
    y-axis "품질 특성값" 0 --> 10

    line "UCL" [8,8,8,8,8,8]
    line "CL" [5,5,5,5,5,5]
    line "LCL" [2,2,2,2,2,2]

    line "측정값" [6,4,5,7,3,6]
```
...
1. **Cp (Process Capability Index)**
    ...
    **공정 평균**이 규격 중심과 일치한다고 가정한 잠재 공정능력이다.
...
**실험계획법(DOE, Design of Experiments)**은 실험계획법은 최소의 실험 횟수로 최고 품질의 정밀한 정보를 얻을 수 있도록 실험을 설계하고, 그 결과를 통계적으로 분석하여 공정이나 제품의 최적 조건을 도출하는 정량적 분석 기법이다. Fisher가 농업 실험에서 체계화하였으며, 제조 공정 최적화에 널리 활용된다.

* **주요 인자 선별(Screening)**  
    수많은 공정 변수 중에서 출력 결과(반응치)에 유의미한 영향을 미치는 인자(Factor)를 식별
* **최적화(Optimization)**  
    목표로 하는 반응치를 얻기 위한 최적의 인자 수준(Level) 조합 설정
* **강건 설계(Robust Design)**  
    온도, 습도 등 제어 불가능한 잡음 인자(Noise)의 영향에 흔들리지 않는 안정적인 조건 탐색

**실험계획의 3대 기본 원칙**

실험 결과의 신뢰성과 통계적 타당성을 확보하기 위해 반드시 준수해야 하는 기준이다.

1. **랜덤화의 원칙(Randomization)**  
    실험 순서나 배치를 무작위로 결정하여 외생 변수나 시차에 의한 오차가 결과에 미치는 체계적 영향을 제거한다.
2. **반복의 원칙(Replication)**  
    동일한 조건에서 실험을 2회 이상 수행하여 오차의 분산(실험 오차)을 정밀하게 추정하고 결과의 신뢰성을 높인다.
3. **블록화의 원칙(Blocking)**  
    실험 환경을 동일한 성격의 블록으로 나눔으로써 파악하고자 하는 핵심 인자의 영향과 제어하기 힘든 외부 변수의 영향을 분리한다.

**주요 실험계획법의 종류**

연구의 목적, 분석 단계, 허용 가능한 실험 비용에 따라 적절한 배치 방식을 선택한다.

1. **요인 배치법(Factorial Design)**  
...
2. **직교배열표에 의한 배치(Orthogonal Array)**  

    다수 인자를 적은 횟수의 실험으로 효율적으로 분석하기 위해 직교성을 가진 수학적 표를 활용하는 방식이다.
    
    * 특징  
        타구치 기법(Taguchi Method) 등에서 효율적인 공정 최적화 및 비용 절감을 목적으로 광범위하게 활용

3. **반응표면 분석법(RSM, Response Surface Methodology)**  

    인자가 연속형 변수일 때 반응치와의 곡선 형태 관계(2차식 등)를 모델링하여 정밀한 최적점을 찾는 기술이다.
    
    * 대표 배치법  
        중심합성계획법(CCC, Central Composite Design), 박스-벤켄 계획법(Box-Behnken Design)

**주요 DOE 배치법 비교**

| 구분 | 완전 요인 배치법 | 일부 요인 배치법 | 반응표면 분석법(RSM) |
| --- | --- | --- | --- |
| **주요 목적** | 인자의 효과 및 상호작용 완전 파악 | 핵심 인자 선별(Screening) | 정밀한 곡선 모델링 및 최적화 |
| **실험 횟수** | 많음 ($L^k$, 인자 수준승) | 적음 ($L^{k-p}$) | 중간 이상 (중심점 포함) |
| **상호작용 분석** | 모든 주효과 및 교호작용 검증 가능 | 고차 교호작용 교송(Aliasing) 발생 | 2차 항목 및 곡선 효과 검증 가능 |

**DOE 수행 6단계 절차**

```
[1단계: 목적 정의] -> [2단계: 반응치 및 인자 선정] -> [3단계: 실험 계획 및 수행] 
                 -> [4단계: 통계 분석(ANOVA)] -> [5단계: 최적화 및 예측] -> [6단계: 확인 실험]
```

1. **목적 정의**: 달성하고자 하는 품질목표 설정
2. **반응치($Y$) 및 인자($X$) 선정**: 측정 가능한 결과값과 제어할 원인 변수 및 수준 확정
3. **실험 설계 및 데이터 수집**: 적합한 DOE 배치법을 선택하고 무작위로 실험 수행
4. **통계적 분석**: 분산분석(ANOVA)을 활용하여 유의성 평가 및 회귀 모델 구축
5. **최적 조건 도출**: 반응치 목표를 충족하는 인자별 수준 조건 계산
6. **확인 실험(Confirmation Run)**: 도출된 최적 조건에서 재실험을 거쳐 모델 재현성 검증
...
flowchart TD
    ...
    A["측정시스템 분석<br/>(MSA : Measurement System Analysis)"]:::main


    B["정확도<br/>(Accuracy)"]:::category
    C["정밀도<br/>(Precision)"]:::category
    ...
    B1["Bias<br/>(편의)"]:::detail
    B2["Linearity<br/>(선형성)"]:::detail
    B3["Stability<br/>(안정성)"]:::detail


    C1["Repeatability<br/>(반복성)"]:::detail
    C2["Reproducibility<br/>(재현성)"]:::detail
...
![](https://github.com/user-attachments/assets/8859db11-7ebe-4185-9616-c8508c73f2d4)
...
![https://www.jksqm.org/upload/thumbnails/jksqm-44-2-277f1.gif](https://www.jksqm.org/upload/thumbnails/jksqm-44-2-277f1.gif)
...
```mermaid
flowchart LR
    classDef old fill:#FFEBEE,stroke:#D32F2F,stroke-width:2px,color:#000,font-weight:bold;
    classDef new fill:#E8F5E9,stroke:#2E7D32,stroke-width:2px,color:#000,font-weight:bold;
    classDef title fill:#E3F2FD,stroke:#1976D2,stroke-width:2px,color:#000,font-weight:bold;


    A["제조 현장 사례<br/>(설비 고장 관리)"]:::title


    B["과거<br/>ISO 9001:2008<br/>(사후 대응 중심)"]:::old

    B1["설비 고장 발생<br/>(Failure Occurrence)"]:::old
    B2["재발 방지 대책 수립<br/>(Corrective Action)"]:::old


    C["현재<br/>ISO 9001:2015<br/>(리스크 기반 사고)"]:::new

    C1["설비 고장 가능성 분석<br/>(Risk Analysis)"]:::new
    C2["예방보전 계획 수립<br/>(Preventive Maintenance Plan)"]:::new
    C3["고장 발생 전에 관리<br/>(Risk Prevention)"]:::new
