...
```
[운송 관리] ───> [창고/재고 관리] ───> [하역/하역 기계화] ───> [포장 관리] ───> [물류 정보]
```
...
#### 재고 및 풀필먼트 관리 기법 (Inventory & Fulfillment)

재고 및 풀필먼트 관리 기법 (Inventory & Fulfillment)은 다음과 같다.
...
재고관리(Inventory Management)는 기업이 원자재, 반제품, 완제품 등 자재의 흐름을 효율적으로 통제하여 **고객의 수요에 적시 대응하는 동시에 재고 보유에 따른 비용을 최소화하는 관리 활동**이다
...
* **수송재고/이동재고(Pipeline/Transit Stock)**  
  공급업체, 공장, 물류센터, 대리점 등 공급망의 한 거점에서 다른 거점으로 운송 중(In-Transit)에 있는 재고입니다.
